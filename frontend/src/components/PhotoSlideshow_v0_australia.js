...
function PhotoSlideshow() {
  ...
  const initializeSocket = useCallback(() => {
    ...
    try {
      ...
      //       (photoData.location === 'Australia' || photoData.location === 'Both Australia and Canada')) {
  ...
  const fetchRandomPhoto = useCallback(async (retryCount = 0) => {
    try {
      const response = await fetch(
        `${API_BASE}/api/photos/random?count=1&location=Australia&seed=${Math.random()}`
      );
  ...
  const fetchPhotoCount = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/photos/count?location=Australia`, {
